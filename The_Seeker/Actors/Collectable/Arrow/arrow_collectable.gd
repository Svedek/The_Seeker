...
func dislodge():
	...
	pos_tween.tween_property(self, "position", (Vector2.DOWN.rotated(rotation + randf_range(-0.25*PI, 0.25*PI)) * (32 + randi() % 48)) , 1.0).as_relative().set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_OUT)
