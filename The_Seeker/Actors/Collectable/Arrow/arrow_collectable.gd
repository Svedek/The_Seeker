...
func expell():
	activate()
	var expell_dir = Vector2.RIGHT.rotated(randf_range(0,2*PI))  # Use this to decide which way to shoot the arrow PEW!!!
	var pos_tween = create_tween().set_parallel(true)
	pos_tween.tween_property(self, "position", (expell_dir * (64 + randi() % 64)) , 1.0).as_relative().set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_OUT)
	pos_tween.tween_property(self, "rotation", randf_range(-1.5*PI, 1.5*PI), 1.0).as_relative().set_trans(Tween.TRANS_CIRC).set_ease(Tween.EASE_OUT)
	await pos_tween.finished
